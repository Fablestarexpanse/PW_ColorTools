...
The nodes appear under **PW Color** in the Add Node menu. For a graph that
already has the whole pack wired, open *Workflow → Browse Templates →
Extensions → PW_ColorTools*.

No dependencies beyond torch, numpy, Pillow and aiohttp — all of which ComfyUI
already has. The web bundle is committed, so a plain clone works with no node
toolchain.
...
the same in the Classic node design and in *Modern Node Design (Nodes 2.0)*.
...
<img src="docs/images/pw_review_wired.png" alt="KSampler, then VAE Decode, then PW Review, then Save Image" width="720">
